...
bool HidController::poll() {
    ...
    // - If the mapping processes the Input Reports too slow, whole Input Reports will be omited if the ring buffer is full
        ...
        if (bytesRead < 0) {
            ...
            for (int rptIdx = 0; rptIdx <= kMaxHidInputReportId; rptIdx++) {
                if (inputReportCounter[rptIdx]>1) {
                    qCWarning(m_logOutput) << getName() << ": Read " << inputReportCounter[rptIdx] << " HID input reports with Report ID " << rptIdx << "in the same polling interval. This indicates a performance problem with controller mapping or a too slow polling interval.";
                }
            }
            return true;
        }
        inputReportCounter[m_pPollData[m_pollingBufferIndex][0]]++; // Count reports read in this while loop per ID
